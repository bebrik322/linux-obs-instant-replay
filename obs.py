...
OBS_SENTINEL_PATH = (
    Path.home() / ".var/app/com.obsproject.Studio/config/obs-studio/.sentinel"
)
# ---------------------
...
def run_obs_in_thread():
    ...
    try:
        ...
        logger.info(
            f"OBS Studio process started with PID: {obs_process.pid}. It will continue running after this script exits."
        )
    except FileNotFoundError:
        logger.error(
            "Error: 'flatpak' command not found. Is Flatpak installed and in your PATH?"
        )
...
def is_obs_responsive():
    ...
    try:
        ...
        result = subprocess.run(
            OBS_CMD_CHECK_COMMAND, check=True, capture_output=True, text=True, timeout=5
        )
        logger.debug(
            f"OBS responsive check successful. Version: {result.stdout.strip()}"
        )
        return True
    except FileNotFoundError:
        logger.error(
            "Error: 'obs-cmd' not found. Please install it (yay -S obs-cmd) and ensure it's in your PATH."
        )
        ...
        logger.debug(
            f"obs-cmd check failed (CalledProcessError): {e.stderr.strip() if e.stderr else e.stdout.strip()}"
        )
        return False
...
def main():
    ...
    obs_thread = threading.Thread(
        target=run_obs_in_thread, name="OBSLauncherThread", daemon=True
    )
    obs_thread.start()
    ...
    if obs_process is None:
        logger.error(
            "OBS process object not created. This likely means 'flatpak' was not found or another critical error occurred in the OBS starting thread."
        )
        logger.info("Script will exit.")
        return
    elif obs_process.poll() is not None:  # Check if process already exited
        logger.error(
            f"OBS process (PID: {obs_process.pid}) started but exited immediately with code {obs_process.returncode}. Check OBS logs or flatpak errors."
        )
        logger.info("Script will exit.")
        return
    else:
    ...
    logger.info(
        f"Waiting for OBS (via obs-websocket) to become responsive (max {MAX_WAIT_TIME_SECONDS} seconds)..."
    )
    ...
    while time.time() - start_time < MAX_WAIT_TIME_SECONDS:
        ...
        if obs_process and obs_process.poll() is not None:
            logger.warning(
                f"OBS process (PID: {obs_process.pid}) seems to have terminated unexpectedly with code {obs_process.returncode} while waiting for responsiveness."
            )
            obs_is_ready = False
            break
    ...
    if not obs_is_ready:
        ...
        if obs_process and obs_process.poll() is None:
            logger.warning(
                "OBS process might still be starting or stuck. You may need to manage it manually."
            )
    ...
    logger.info(
        f"Attempting to toggle replay buffer with command: {' '.join(OBS_CMD_TOGGLE_COMMAND)}"
    )
    try:
        result = subprocess.run(
        ...
        if result.stderr:  # Should be empty on success, but log if present
            logger.warning(
                f"obs-cmd errors (though command succeeded): {result.stderr.strip()}"
            )
    except FileNotFoundError:
        ...
        logger.warning(
            "  This could mean OBS isn't configured for replays, the replay buffer isn't active, or another obs-websocket issue."
        )
    except subprocess.TimeoutExpired:
    ...
    logger.info(
        "OBS Studio (if started successfully) should still be running in the background."
    )
